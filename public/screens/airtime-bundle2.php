...
<body class="hold-transition sidebar-mini layout-fixed">
      ...
      .then((snapshot) => {
        snapshot.forEach((doc) => {
          ...
          for(let i=0;i<usersIds.length;i++){
            if(transactionsData.UserId == usersIds[i]){
              if(transactionsData.type == "Forfaits"){
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h4 class="m-0 text-dark">Achats de forfaits</h4>
            </div><!-- /.col -->
          </div><!-- /.row -->
        </div><!-- /.container-fluid -->
      </div>
          ...
          <div class="col-12">
            <div class="card">
              ...
              <div class="card-body table-responsive p-0">
                <table id="tableau-bundlePurchasedList" class="table table-hover text-nowrap">
                  <thead>
                    <tr>
                      <th>ID</th>
                      <th>Date d'achat</th>
                  ...
                  <tbody>
                    <tr v-for="(data, index) in bundlePurchasedList" :key="data.id">
                      <td> {{ index + 1 }}</td>
                      ...
                      <!--<td> {{ data.type }}</td>-->
                      <td>
                        <button type="button" :class="btns.viewClassValue" data-toggle="modal" @click="viewDetails(data.id,data.date,data.phone,data.title,data.nom,data.prenom)"
                          data-target="#modal-det"><i :class="btns.viewIconValue"></i></button>
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <div class="modal-body">
              ...
              <form role="form">
                <div class="card-body">
                  ...
                  <div class="form-group">
                    <label for="InputFirstName">Opérateur</label>
                    <input readonly type="text" class="form-control" id="operateur">
                  </div>
                  <div class="form-group">
                    <label for="InputTel">Type</label>
                    <input readonly type="text" class="form-control" id="type_forfait">
                  </div>
                  <div class="form-group">
                    <label for="InputPassword1">Montant</label>
                    <input readonly type="text" class="form-control" id="montant">
                  </div>
                  <div class="form-group">
                    <label for="InputPassword2">Nom & Prénom(s) de l'acheteur</label>
                    <input readonly type="text" style="text-transform:uppercase" class="form-control" id="np_acheteur">
                  </div>
                </div>
                <!-- /.card-body -->
              </form>
  ...
  <script>
        ...
        const updateData = () => {
          if (window.myData && window.myData.length > 0) {
            ...
            for(let i=0;i<usersData.length;i++){
              ...
              this.bundlePurchasedList = window.myData;
          } else {
    ...
    viewDetails(id,date,phone,title,nom,prenom){
          this.idOpe = id;
          ...
          document.getElementById('type_forfait').value = this.afficherTypeForfait(title);
          document.getElementById('montant').value = this.afficherMontant(title);
          document.getElementById('np_acheteur').value = nom +" "+prenom;
    },
    ...
    function searchEl(motsCles) {
      var lignes = document.querySelectorAll('#tableau-bundlePurchasedList tbody tr');
