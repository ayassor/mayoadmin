...
if ($decode_users_data !== null && isset($decode_users_data['data'])) {
  ...
  if ($decode_money_trans_data !== null) {
    ...
    foreach ($decode_money_trans_data as $money_trans_enregistrement) {
        $data_money_trans_vue[] = $money_trans_enregistrement;
        $nbre_lignes++;
      }
  } else {
    ...
    $data_money_trans_vue_json = json_encode($data_money_trans_vue[2]['data']);
?>
                ...
                <table id="tableau-moneyTrans" class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    <tr v-for="(data, index) in moneyTransList" :key="data.id">
                      ...
                      <td> {{ data.fee }} F CFA</td>
                      <td> {{ data.total_amount }} F CFA</td>
                      <td v-if="data.status"><span class="text-success" style="font-weight:bold"> {{ data.status }} </span></td>
